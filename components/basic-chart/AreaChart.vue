...
<script>
import uCharts from '@/components/u-charts/u-charts-uniapp.js'; //可以优化放全局 uCharts ==>使用全局
const areaCharts = {},
	optionAs = {};
export default {
	name: 'AreaChart',
	props: {
		...
		canvasId: {
			type: String,
			default: `area_canvas_${Math.ceil(Math.random(5) * 10000)}`
		}
	},
	data() {
		return {};
	...
	methods: {
		showCharts() {
			...
			areaCharts[this.canvasId] = new uCharts(optionAs[this.canvasId]);
		},
		touchstart(e) {
			let that = this;
			areaCharts[this.canvasId].touchLegend(e, { animation: false });
			areaCharts[this.canvasId].scrollStart(e);
			areaCharts[this.canvasId].showToolTip(e, {
				//修改点击事弹出文字
		...
		touchmove(e) {
			areaCharts[this.canvasId].scroll(e);
		},
		touchend(e) {
			areaCharts[this.canvasId].scrollEnd(e);
		},
		changeData({ series, categories }) {
			areaCharts[this.canvasId].updateData({
				series,
				categories
			});
		}
